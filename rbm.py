...
# some custom imports will go here
import zconfig
import utils


class RBM(object):
	""" Restricted Boltzmann Machine implementation using tensorflow, this implementation
	adapted from version by user blackecho on github https://gist.github.com/blackecho/db85fab069bd2d6fb3e7
	"""
	...
	def __init__(self, num_visible , num_hidden, visible_unit_type='bin' , main_dir='rbm' , model_name='rbm_model' ,
		...
		:param visible_unit_type: type of visible units (binary or gaussian)
		...
		:param stddev: optional, default of 0.1, Ignored if visible_unit_type is not 'gauss'
		...
		# maybe of use later, but now i don't much care to learn tf's complicated and poorly documented
		# loggin system and whatnot
	...
	def fit(self, train_set , validation_set=None , restore_previous_model=False):
		...
		:param validation_set: validationset. optional, default None
	...
	def _initialize_tf_utilities_and_ops(self, restore_previous_model):
		...
		Restore a previously trained model if the flag is true

		NOTE: I will not currently implement this
		"""
		#self.tf_merged_summaries = tf.merge_all_summaries() DEPRECATED
	...
	def _run_train_step(self, train_set):
		...
		updates = [self.w_update, self.bh_update, self.bv_update]
	...
	def _run_validation_error_and_summaries(self, epoch, validation_set):
		...
		:return: self
		"""
		result = self.tf_session.run([self.tf_merged_summaries, self.loss_function], feed_dict=self._create_feed_dict(validation_set))
	...
	def _create_feed_dict(self, data):
		...
		:param data: training/validation set of batch
	...
	def _build_model(self):
		...
		self.bh_update = self.bh_.assign_add(self.learning_rate * tf.reduce_mean(hprobs0 - hprobs1, 0))
		self.bv_update = self.bv_.assign_add(self.learning_rate * tf.reduce_mean(self.input_data - vprobs, 0))

		self.loss_function = tf.sqrt(tf.reduce_mean(tf.square(self.input_data - vprobs)))
		#_ = tf.scalar_summary("cost" , self.loss_function) SEEMS TO BE DEPRECATED
	...
	def _create_variables(self):
		...
		:return: tuple(weights(shape(num_visible, num_hidden),
						hidden_bias(shape(num_hidden)),
						visible_bias(shape(num_visible)))
		"""

		W = tf.Variable(tf.random_normal([self.num_visible, self.num_hidden] , mean=0.0, stddev=0.01) , name="weights")
		bh_ = tf.Variable(tf.zeros([self.num_hidden]) , name='hidden_bias')
		bv_ = tf.Variable(tf.zeros([self.num_visible]) , name='visible_bias')
	...
	def gibbs_sampling_step(self, visible):
		...
		:return tuple(hidden probs, hidden states, visible probs, new hidden probs, new hidden states)
		"""

		hprobs, hstates = self.sample_hidden_from_visible(visible)
		vprobs = self.sample_visible_from_hidden(hprobs)
		hprobs1, hstates1 = self.sample_hidden_from_visible(vprobs)  # what is the point of this?
	...
	def sample_visible_from_hidden(self, hidden):
		""" Sample the visible units from the hidden units. This is the Negative phase of the Contrastive
		Divergence algorithm

		:param hidden: activations of the hidden units

		:return: visible probabilites
		"""
	...
	def compute_positive_association(self, visible, hidden_probs, hidden_states):
		""" Compute positive associates between visible and hidden units.
	...
	def _create_data_directories(self):
		""" Create the three directories for storing respecitvely, the models,
		the data generated by training, and the TensorFlow's summaries

		:return: tuple of strings(models_dir, data_sir, summary_dir)
		"""

		# just makes sure there is a trailing / on specified directory name ('rbm' is default)
		self.main_dir = self.main_dir + '/' if self.main_dir[-1] != '/' else self.main_dir

		# path to  the current working directory
		dir_path = os.path.dirname(os.path.realpath(__file__))

		# ensures there is a / appended to dir_path
		dir_path = dir_path + '/' if dir_path[-1] != '/' else dir_path

		# ensure that the directory exists
		data_directory_path = dir_path + self.main_dir

		if not os.path.isdir(data_directory_path): os.mkdir(data_directory_path)

		models_dir = data_directory_path + zconfig.models_dir
		data_dir = data_directory_path + zconfig.data_dir
		summary_dir = data_directory_path + zconfig.summary_dir
	...
	def transform(self , data, name='train' , save=False):
		...
		:type save: boolean, default to 'False'
	...
	def load_model(self, shape , gibbs_sampling_steps , model_path):
		""" Load a trained model from disk. the shape of the model (num_visible, num_hidden) and
		the number of gibbs sampling steps must be known in order to restore the model

		:param shape: tuple(num_visible, num_hidden)
		:param gibbs_sampling_steps:
		:param model_path: should be path to dir_name/models/name_of_model
